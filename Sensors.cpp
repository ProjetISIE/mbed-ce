...
bool Sensors::init_bme280() {
    ...
    cmd[0] = 0xF2; cmd[1] = 0x01; // ctrl_hum
    _i2c.write(BME280_ADDR, cmd, 2);
    cmd[0] = 0xF4; cmd[1] = 0x27; // ctrl_meas
    _i2c.write(BME280_ADDR, cmd, 2);
...
bool Sensors::read_sht31(float &temp, float &hum) {
    ...
    if (_i2c.write(SHT31_ADDR, cmd, 2) != 0) return false;
    thread_sleep_for(20);

    char data[6];
    if (_i2c.read(SHT31_ADDR, data, 6) != 0) return false;
...
bool Sensors::read_bme280(float &temp, float &hum) {
    char cmd[1] = {0xF7};
    char data[8];
    if (_i2c.write(BME280_ADDR, cmd, 1) != 0) return false;
    if (_i2c.read(BME280_ADDR, data, 8) != 0) return false;
    ...
    int32_t var1, var2;
    var1 = ((((adc_T >> 3) - ((int32_t)_bme280_comp.dig_T1 << 1))) * ((int32_t)_bme280_comp.dig_T2)) >> 11;
    var2 = (((((adc_T >> 4) - ((int32_t)_bme280_comp.dig_T1)) * ((adc_T >> 4) - ((int32_t)_bme280_comp.dig_T1))) >> 12) * ((int32_t)_bme280_comp.dig_T3)) >> 14;
    ...
    v_x1_u32r = (((((adc_H << 14) - (((int32_t)_bme280_comp.dig_H4) << 20) - (((int32_t)_bme280_comp.dig_H5) * v_x1_u32r)) +
                  ((int32_t)16384)) >> 15) * (((((((v_x1_u32r * ((int32_t)_bme280_comp.dig_H6)) >> 10) *
                                                 (((v_x1_u32r * ((int32_t)_bme280_comp.dig_H3)) >> 11) + ((int32_t)32768))) >> 10) + ((int32_t)2097152)) *
                                               ((int32_t)_bme280_comp.dig_H2) + 8192) >> 14));
    v_x1_u32r = (v_x1_u32r - (((((v_x1_u32r >> 15) * (v_x1_u32r >> 15)) >> 7) * ((int32_t)_bme280_comp.dig_H1)) >> 4));
...
void Sensors::read_all(float &temp1, float &hum1, float &temp2, float &hum2) {
    if (!read_sht31(temp1, hum1)) {
        temp1 = -100.0f; hum1 = -1.0f;
    }
    if (!read_bme280(temp2, hum2)) {
        temp2 = -100.0f; hum2 = -1.0f;
    }
}
...
void Sensors::get_average_temp_and_humidity(float &avg_temp, float &avg_hum) {
    ...
    if (t1 > -50.0f) { avg_temp += t1; avg_hum += h1; count++; }
    if (t2 > -50.0f) { avg_temp += t2; avg_hum += h2; count++; }
